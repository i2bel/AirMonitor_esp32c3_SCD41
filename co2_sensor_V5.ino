...
float tempCalibration = 0.0;     // Смещение для температуры 
float humCalibration = 0.0;     // Смещение для влажности
int co2Calibration = 0;            // Смещение для CO2 

// Флаг, указывающий, что история уже загружена (после синхронизации времени)
...
void saveCalibration(float temp, float hum, int co2);
...
void loadHistoryFromFS() {
  ...
  // Загружаем историю только если время синхронизировано
  if (now < 100000) {
    Serial.println("Time not synced, history loading deferred.");
    return;
  }

  time_t cutoffTime = now - 24 * 3600;
  File file;
  ...
  if (file) {
    ...
    if (fileSize == sizeof(HistoryState)) {
      ...
      if (file.read((uint8_t*)&co2State, sizeof(co2State)) == sizeof(co2State)) {
        bool valid = (co2State.index >= 0 && co2State.index < HISTORY_SIZE &&
                      co2State.count >= 0 && co2State.count <= HISTORY_SIZE);
        if (!valid) {
          Serial.println("CO2 history indices corrupted, starting fresh");
          co2Index = 0; co2Count = 0;
          memset(co2History, 0, sizeof(co2History));
        } else {
          // Статический буфер вместо локального
          static HistoryPoint tempBuffer[HISTORY_SIZE];
          ...
          memcpy(co2History, tempBuffer, sizeof(co2History));
          co2Index = newIndex;
          co2Count = newCount;
      ...
      Serial.printf("CO2 file size mismatch: expected %d, got %d. Starting fresh.\n", sizeof(HistoryState), fileSize);
      co2Index = 0; co2Count = 0;
      memset(co2History, 0, sizeof(co2History));
    }
    file.close();
  ...
  if (file) {
    ...
    if (fileSize == sizeof(HistoryState)) {
      ...
      if (file.read((uint8_t*)&tempState, sizeof(tempState)) == sizeof(tempState)) {
        bool valid = (tempState.index >= 0 && tempState.index < HISTORY_SIZE &&
                      tempState.count >= 0 && tempState.count <= HISTORY_SIZE);
        if (!valid) {
          Serial.println("Temp history indices corrupted, starting fresh");
          ...
          memcpy(tempHistory, tempBuffer, sizeof(tempHistory));
          tempIndex = newIndex;
          tempCount = newCount;
      ...
      Serial.printf("Temp file size mismatch: expected %d, got %d. Starting fresh.\n", sizeof(HistoryState), fileSize);
      tempIndex = 0; tempCount = 0;
      memset(tempHistory, 0, sizeof(tempHistory));
    }
    file.close();
  ...
  if (file) {
    ...
    if (fileSize == sizeof(HistoryState)) {
      ...
      if (file.read((uint8_t*)&humState, sizeof(humState)) == sizeof(humState)) {
        bool valid = (humState.index >= 0 && humState.index < HISTORY_SIZE &&
                      humState.count >= 0 && humState.count <= HISTORY_SIZE);
        if (!valid) {
          Serial.println("Hum history indices corrupted, starting fresh");
          humIndex = 0; humCount = 0;
          ...
          memcpy(humHistory, tempBuffer, sizeof(humHistory));
          humIndex = newIndex;
          humCount = newCount;
      ...
      Serial.printf("Hum file size mismatch: expected %d, got %d. Starting fresh.\n", sizeof(HistoryState), fileSize);
      humIndex = 0; humCount = 0;
      memset(humHistory, 0, sizeof(humHistory));
    }
    file.close();
...
bool readSensor() {
  ...
  if (measurementStarted) {
    ...
    currentData.temperature = temp + tempCalibration; // Калибровка из переменной
    currentData.humidity = hum + humCalibration;      // Калибровка из переменной
    currentData.co2 = co2 + co2Calibration;           // Если хочешь калибровать и CO2
...
void publishDiscoveryConfigs() {
  ...
  serializeJson(co2Config, buffer, sizeof(buffer));
  ...
  serializeJson(tempConfig, buffer, sizeof(buffer));
  ...
  serializeJson(humConfig, buffer, sizeof(buffer));
  ...
  serializeJson(rssiConfig, buffer, sizeof(buffer));
...
<script>
    ...
    co2Options.scales.y.ticks = { callback: function(value) { return Math.round(value); } };

    let tempOptions = JSON.parse(JSON.stringify(baseOptions));
    tempOptions.scales.y.ticks = { callback: function(value) { return value.toFixed(2); } };

    let humOptions = JSON.parse(JSON.stringify(baseOptions));
    humOptions.scales.y.ticks = { callback: function(value) { return value.toFixed(2); } };
...
<body>
    ...
    .then(data => {
        statusDiv.innerText = 'Update started! Rebooting...';
        setTimeout(() => window.location.href = '/', 5000);
    })
    .catch(err => {
        // Даже если ошибка - просто ждём перезагрузки
        statusDiv.innerText = 'Update in progress... Device will reboot.';
        setTimeout(() => window.location.href = '/', 5000);
    });
});
    </script>
...
void setupWebServer() {
  ...
  webServer.on("/update", HTTP_POST, [](AsyncWebServerRequest *request) {
        ...
        AsyncWebServerResponse *response = request->beginResponse(200, "text/html", 
            "<!DOCTYPE html><html><body style='font-family:-apple-system;text-align:center;padding:40px;'>"
            "<h2>Update Started!</h2>"
            "<p>Device will reboot in 5 seconds...</p>"
            "<script>setTimeout(()=>{window.location.href='/';},5000);</script>"
            "</body></html>");
        response->addHeader("Connection", "close");  // Важно!
        request->send(response);
        
        Serial.println("OTA update started - will reboot after completion");
    }, [](AsyncWebServerRequest *request, String filename, size_t index, uint8_t *data, size_t len, bool final) {
        // НИКАКИХ БЛОКИРУЮЩИХ ВЫЗОВОВ ЗДЕСЬ!
        ...
        if (final) {
            if (Update.end(true)) {
                ...
                shouldReboot = true;  // Просто устанавливаем флаг
            } else {
                Update.printError(Serial);
            }
        }
    });
    // СКРЫТАЯ СТРАНИЦА КОНФИГУРАЦИИ
webServer.on("/config", HTTP_GET, [](AsyncWebServerRequest *request) {
  String html = R"CONFIG_PAGE(
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Air Monitor - Config</title>
    <style>
        *{margin:0;padding:0;box-sizing:border-box;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;}
        body{background:linear-gradient(145deg,#f0f4fa 0%,#d9e2ef 100%);min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;}
        .card{background:rgba(255,255,255,0.6);backdrop-filter:blur(20px);border-radius:40px;padding:30px;box-shadow:0 20px 40px rgba(0,0,0,0.08);max-width:600px;width:100%;}
        h1{font-size:32px;font-weight:600;color:#1c1c1e;margin-bottom:20px;}
        h2{font-size:18px;font-weight:500;color:#1c1c1e;margin:20px 0 10px;}
        .section{background:rgba(255,255,255,0.4);backdrop-filter:blur(15px);border-radius:30px;padding:22px;margin-bottom:20px;}
        label{display:block;margin:10px 0 5px;color:#6c6c70;font-size:14px;}
        input{width:100%;padding:12px;border:1px solid #d9d9dc;border-radius:14px;font-size:16px;background:#f9f9fb;margin-bottom:10px;}
        input:focus{border-color:#007aff;outline:none;}
        .note{color:#8e8e93;font-size:12px;margin-top:-5px;margin-bottom:10px;}
        button{background:#007aff;color:white;border:none;padding:14px;border-radius:40px;font-size:16px;width:100%;cursor:pointer;}
        .footer{text-align:center;margin-top:20px;color:#8e8e93;font-size:14px;}
        .footer a{color:#007aff;text-decoration:none;}
        hr{border:none;border-top:1px solid rgba(255,255,255,0.6);margin:20px 0;}
    </style>
</head>
<body>
    <div class="card">
        ...
        <form id="configForm">
            <div class="section">
                ...
                <input type="number" step="0.1" name="temp" id="temp" value=")CONFIG_PAGE" + String(tempCalibration) + R"CONFIG_PAGE(">
                <div class="note">Current: <span id="calibTemp">---</span>°C</div>
                
                <label>Humidity Offset (%)</label>
                <input type="number" step="0.1" name="hum" id="hum" value=")CONFIG_PAGE" + String(humCalibration) + R"CONFIG_PAGE(">
                <div class="note">Current: <span id="calibHum">---</span>%</div>
                
                <label>CO2 Offset (ppm)</label>
                <input type="number" step="1" name="co2" id="co2" value=")CONFIG_PAGE" + String(co2Calibration) + R"CONFIG_PAGE(">
                <div class="note">Current: <span id="calibCo2">---</span>ppm</div>
            </div>
            ...
            <div class="section">
                ...
                <label>SSID</label>
                <input type="text" name="wifi_ssid" value=")CONFIG_PAGE" + wifiSSID + R"CONFIG_PAGE(">
                
                <label>Password</label>
                <input type="password" name="wifi_pass" value=")CONFIG_PAGE" + wifiPass + R"CONFIG_PAGE(">
            </div>
            ...
            <div class="section">
                ...
                <label>Server</label>
                <input type="text" name="mqtt_server" value=")CONFIG_PAGE" + mqttServer + R"CONFIG_PAGE(">
                
                <label>Port</label>
                <input type="number" name="mqtt_port" value=")CONFIG_PAGE" + String(mqttPort) + R"CONFIG_PAGE(">
                
                <label>Username</label>
                <input type="text" name="mqtt_user" value=")CONFIG_PAGE" + mqttUser + R"CONFIG_PAGE(">
                
                <label>Password</label>
                <input type="password" name="mqtt_pass" value=")CONFIG_PAGE" + mqttPass + R"CONFIG_PAGE(">
            </div>
        ...
        <div class="footer">
            Device ID: )CONFIG_PAGE" + deviceId + R"CONFIG_PAGE(<br>
    ...
    function updateValues() {
        fetch('/api/data')
            .then(r => r.json())
            .then(data => {
                if (data.temperature) document.getElementById('calibTemp').innerText = data.temperature.toFixed(1);
                if (data.humidity) document.getElementById('calibHum').innerText = data.humidity.toFixed(1);
                if (data.co2) document.getElementById('calibCo2').innerText = data.co2;
            });
    }
    ...
    document.getElementById('configForm').addEventListener('submit', function(e) {
        ...
        if (!statusDiv) {
            ...
            statusDiv.style.borderRadius = '14px';
            statusDiv.style.textAlign = 'center';
            this.appendChild(statusDiv);
        }
        statusDiv.innerText = 'Saving settings...';
        statusDiv.style.background = '#007aff20';
        ...
        fetch('/save_config', { method: 'POST', body: formData })
            .then(r => r.text())
            .then(data => {
                statusDiv.innerText = 'Settings saved. Rebooting in 3 seconds...';
                statusDiv.style.background = '#34c75920';
            ...
            .catch(err => {
                statusDiv.innerText = 'Error saving settings';
                statusDiv.style.background = '#ff3b3020';
...
webServer.on("/save_config", HTTP_POST, [](AsyncWebServerRequest *request) {
  ...
  if (request->hasParam("temp", true) && request->hasParam("hum", true) && request->hasParam("co2", true)) {
    float temp = request->getParam("temp", true)->value().toFloat();
    float hum = request->getParam("hum", true)->value().toFloat();
    int co2 = request->getParam("co2", true)->value().toInt();
    saveCalibration(temp, hum, co2);
  }
  ...
  if (request->hasParam("wifi_ssid", true)) {
    ...
    String mqttPass = request->getParam("mqtt_pass", true)->value();
    
    saveConfig(wifiSsid, wifiPass, mqttServer, mqttPort, mqttUser, mqttPass);
  }
  
  // 👇 ВАЖНО: отправляем ответ и разрешаем клиенту его получить
  AsyncWebServerResponse *response = request->beginResponse(200, "text/plain", "OK");
  response->addHeader("Connection", "close");
  request->send(response);
  
  // 👉 Используем существующий флаг shouldReboot
  shouldReboot = true;
  rebootTime = millis();  // нужно добавить переменную
  
  Serial.println("Settings saved, reboot scheduled");
});
  webServer.begin();
...
void loadCalibration() {
  ...
  tempCalibration = preferences.getFloat("temp", 0.0);
  humCalibration = preferences.getFloat("hum", 0.0);
  co2Calibration = preferences.getInt("co2", 0);
  preferences.end();
  Serial.printf("Calibration loaded: temp %+.1f, hum %+.1f, co2 %+d\n", 
                tempCalibration, humCalibration, co2Calibration);
}
...
void saveCalibration(float temp, float hum, int co2) {
  ...
  preferences.putInt("co2", co2);
  preferences.end();
  tempCalibration = temp;
  humCalibration = hum;
  co2Calibration = co2;
  Serial.printf("Calibration saved: temp %+.1f, hum %+.1f, co2 %+d\n", 
                temp, hum, co2);
}
...
void setup() {
  ...
  if (!LittleFS.begin()) {
    ...
    if (LittleFS.format()) {
      ...
      if (!LittleFS.begin()) {
        Serial.println("ERROR: Cannot mount LittleFS even after format");
      } else {
        Serial.println("LittleFS mounted after format");
        // История будет загружена позже, когда синхронизируется время
      }
    } else {
...
void loop() {
  ...
  // Проверяем, нужно ли перезагрузиться после OTA
    if (shouldReboot) {
    if (rebootTime == 0) {
        rebootTime = millis();  // защита от случайностей
    }
    if (millis() - rebootTime > 3000) {  // ждём 3 секунды
        ESP.restart();
    }
}
  ...
  if (millis() - lastHistoryUpdate >= HISTORY_INTERVAL) {
    ...
    if (now < 100000) {
      ...
      // Добавляем усреднённые данные за прошедший интервал
    ...
    // Сбрасываем накопленные суммы и счётчик для следующего интервала
